...
class HorizontalAdvectionTerm(TracerTerm):
    ...
    def residual(self, solution, solution_old, fields, fields_old, bnd_conditions=None):
        ...
        uv = fields_old['uv_3d']
        uv_elem = uv.function_space().ufl_element()
        hdiv = element_continuity(uv_elem).horizontal == 'hdiv'
        ...
        if not hdiv:
            ...
            f += inner(div(solution*uv), self.test)*self.dx
            #f += solution*(uv[0]*self.normal[0] + uv[1]*self.normal[1])*self.test*ds_surf
        return -f
...
class VerticalAdvectionTerm(TracerTerm):
    ...
    def residual(self, solution, solution_old, fields, fields_old, bnd_conditions=None):
        ...
        w_elem = w.function_space().ufl_element()
        hdiv = element_continuity(w_elem).horizontal == 'hdiv'
        ...
        if not hdiv:
            ...
            f += Dx(solution*vertvelo, 2)*self.test*self.dx
            #f += solution*vertvelo*self.normal[2]*self.test*self.ds_surf
        return -f
